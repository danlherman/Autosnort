...
if [[ "$release" -ge "7" ]]; then
	...
	declare -a packages=( ethtool make zlib-devel gcc libtool pcre-devel libdnet-devel libpcap-devel mysql-devel perl perl-Crypt-SSLeay perl-libwww-perl perl-Archive-Tar perl-IO-Socket-SSL perl-Sys-Syslog perl-LWP-Protocol-https flex bison autoconf );
	install_packages ${packages[@]}
fi
